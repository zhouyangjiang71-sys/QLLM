...
class Q_Corrector(nn.Module):
    def __init__(self, args):
        super(Q_Corrector, self).__init__()
        self.args = args
        self.n_agents = args.n_agents
        self.state_dim = int(np.prod(args.state_shape))
        self.hyper_w = nn.Sequential(nn.Linear(self.state_dim, self.args.hypernet_embed),
                                       nn.ReLU(inplace=True),
                                       nn.Linear(self.args.hypernet_embed, self.n_agents))
        self.hyper_b = nn.Sequential(nn.Linear(self.state_dim, self.args.hypernet_embed),
                                       nn.ReLU(inplace=True),
                                       nn.Linear(self.args.hypernet_embed, self.n_agents))
    def forward(self, agent_qs, states):
        ...
        w1 = self.hyper_w(states)
        b1 = self.hyper_b(states)
        w1 = w1.view(-1, self.n_agents)
        b1 = b1.view(-1, self.n_agents)
        Correct_q = agent_qs * w1 + b1
        return Correct_q.reshape(batch_size, seq_len, -1)
...
class NQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        print(args.name+args.env_args['key']+',Parameter Size: ')
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        with th.no_grad():
            ...
            if self.args.mixer == "vdn":
                target_max_qvals = self.target_corrector(target_max_qvals, batch["state"])
            target_max_qvals = self.target_mixer(target_max_qvals, batch["state"])
            if getattr(self.args, 'q_lambda', False):
                qvals = th.gather(target_mac_out, 3, batch["actions"]).squeeze(3)
                if self.args.mixer == "vdn":
                    qvals = self.target_corrector(qvals, batch["state"])
        ...
        if self.args.mixer == "vdn":
            chosen_action_qvals = self.corrector(chosen_action_qvals, batch["state"][:, :-1])
        # Mixer
    ...
    def _update_targets(self):
        ...
        if self.args.mixer == "vdn":
            self.target_corrector.load_state_dict(self.corrector.state_dict())
