...
class ForagingEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        for row in range(rows):
            for col in range(cols):
                if self.field[row, col] > 0:
                    self.initial_foods.append((row, col, self.field[row, col]))
    ...
    def get_state(self):
        state = []
        for player in self.players:
            x, y = player.position  # 根据实际属性名调整
            level = player.level  # 根据实际属性名调整
            state += [x, y, level]
        for (row, col, init_level) in self.initial_foods:
            current_level = self.field[row, col]
            if current_level > 0:
                state += [row, col, current_level]
            else:
                state += [-1, -1, 0]
        return state + self.last_actions
    def state_size(self):
        return self.n_agents*4+self.max_num_food*3
